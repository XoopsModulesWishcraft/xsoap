...
function compile_wsdl(){
	...
	while ($row = $xoopsDB->fetchArray($ret)){
		...
		WSDL_dump_File(get_xsd($row['plugin_name']),XOOPS_ROOT_PATH."/modules/xsoap/".$row['plugin_name'].".xsd");
		
		WSDL_dump_File(get_wsdl($row['plugin_name']),XOOPS_ROOT_PATH."/modules/xsoap/".$row['plugin_name'].".wsdl");
			
		WSDL_dump_File(get_wsdl_service($row['plugin_name']),XOOPS_ROOT_PATH."/modules/xsoap/".$row['plugin_name'].".service.wsdl");
