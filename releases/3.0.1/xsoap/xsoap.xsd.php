...
if (!defined('wsdl_local_compile')&&$_GET['funcname']!=''&&file_exists(XOOPS_ROOT_PATH.'/modules/xsoap/plugins/'.$_GET['funcname'].'.php')){
	...
	function get_xsd($func){
		...
		if (function_exists($funcxsd)){
			...
			$rst .=  '<schema targetNamespace="'.XOOPS_URL.'/modules/xsoap/'.$func.'/schemas" xmlns="http://www.w3.org/2000/10/XMLSchema">'.chr(13);
			$rst .=  chr(9).'<element name="'.$func.'Request">'.chr(13);
			$rst .=  chr(9).chr(9).'<complexType>'.chr(13);
			$rst .=  chr(9).chr(9).chr(9).'<all>'.chr(13);				
			ob_start();	
			foreach ($xsd['request'] as $request){
				$rst .=  $ele->output($request, chr(9).chr(9).chr(9).chr(9));	
			}	
			$rst .=  ob_get_contents();
			ob_end_clean();
			$rst .=  chr(9).chr(9).chr(9).'</all>'.chr(13);				
			$rst .=  chr(9).chr(9).'</complexType>'.chr(13);
			$rst .=  chr(9).'</element>'.chr(13);
			
			$rst .=  chr(9).'<element name="'.$func.'">'.chr(13);
			$rst .=  chr(9).chr(9).'<complexType>'.chr(13);
			$rst .=  chr(9).chr(9).chr(9).'<all>'.chr(13);
			ob_start();				
			foreach ($xsd['response'] as $response){
				$rst .=  $ele->output($response, chr(9).chr(9).chr(9).chr(9));	
			}	
			$rst .=  ob_get_contents();
			ob_end_clean();
			$rst .=  chr(9).chr(9).chr(9).'</all>'.chr(13);				
