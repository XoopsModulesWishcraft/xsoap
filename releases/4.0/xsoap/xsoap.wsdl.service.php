...
if (!defined('wsdl_local_compile')&&$_GET['funcname']!=''&&file_exists(XOOPS_ROOT_PATH.'/modules/xsoap/plugins/'.$_GET['funcname'].'.php')){
	...
	function get_wsdl_service($function){;
		...
		if (function_exists($funcwsdl)){
			...
			$rst .=  '<definitions name="'.$function.'" targetNamespace="'.XOOPS_URL.'/modules/xsoap/'.$function.'/service" xmlns:tns="'.XOOPS_URL.'/modules/xsoap/'.$function.'/service" xmlns:soap="http://schemas.xmlsoap.org/wsdl/soap/" xmlns:defs="'.XOOPS_URL.'/modules/xsoap/'.$function.'/definitions"  xmlns="http://schemas.xmlsoap.org/wsdl/">'.chr(13);
			...
			$rst .=  chr(9).'<service name="'.$function.'Service">'.chr(13);
