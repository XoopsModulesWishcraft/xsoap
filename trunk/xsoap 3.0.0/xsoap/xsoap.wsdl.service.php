...
if ($_GET['funcname']!=''&&file_exists(XOOPS_ROOT_PATH.'/modules/xsoap/plugins/'.$_GET['funcname'].'.php')){
	...
	if (function_exists($funcwsdl)){
		...
		echo '<definitions name="'.$function.'" targetNamespace="'.XOOPS_URL.'/modules/xsoap/wsdl/'.$function.'/service" xmlns:tns="'.XOOPS_URL.'/modules/xsoap/wsdl/'.$function.'/service" xmlns:soap="http://schemas.xmlsoap.org/wsdl/soap/" xmlns:defs="'.XOOPS_URL.'/modules/xsoap/wsdl/'.$function.'/definitions"  xmlns="http://schemas.xmlsoap.org/wsdl/">'.chr(13);
